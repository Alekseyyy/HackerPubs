...
namespace FP
{
    class FingerPrintManager
    {
        ...
        static Dictionary<string, MethodInfo> _evaluateStringSetMethods = new Dictionary<string, MethodInfo>();
        ...
        static private void Clear()
        {
            ...
            _evaluateStringSetMethods.Clear();
        ...
        static public bool LoadFingerPrints(string theDirectory)
        {
            ...
            foreach (string file in theFiles)
            {
                ...
                // we only want .cs files
                if (sourceFile.Extension.ToLower(CultureInfo.InvariantCulture) != ".cs")
                {
                    continue;
                }
            ...
            //
            // Compile each fingerprint and create instances of each
            //
                    ...
                    foreach (Type type in types)
                    {
                        if ((null != type.GetInterface("IFingerPrint")) || (type.BaseType.Name == "BaseFingerPrint"))
                        {
                            ...
                            foreach (MethodInfo method in methods)
                            {
                                ...
                                else if (method.Name == "OnEvaluateStringSet")
                                {
                                    _evaluateStringSetMethods[type.FullName] = method;
                                    bEvalSet = true;
                                }
                            ...
                            if (bLoad && bRegister && bMatch && bEval && bEvalSet && bScan && bScanComplete)
                            {
        ...
        static public void OnEvaluateStringSet(Dictionary<string, bool> stringSet, ScanResultCollection results)
        {
            foreach (KeyValuePair<string, bool> pair in _loadResults)
            {
                if (pair.Value == true)
                {
                    if (_objectInstances.ContainsKey(pair.Key))
                    {
                        ...
                        if (_evaluateStringSetMethods.ContainsKey(pair.Key))
                        {
                            MethodInfo method = _evaluateStringSetMethods[pair.Key];

                            object result = method.Invoke(theObject, new object[] { stringSet, results });
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Compile all the source files into a single assembly 
        /// </summary>
        /// <returns></returns>
        static private bool CompileAssembly(out CompilerResults results)
        {
            ...
            if (null == _provider)
            {
                ...
                // TODO: determine required files on the fly
                ...
                _requiredAssemblies.Add("mscorlib.dll");
            ...
            // For release use these options for performance
            ...
            // For debugging use these options to allow breakpoints/single stepping into fingerprints
