...
namespace FP
{
    #region XmlSerializableDictionary
    ...
    public class ScanResultCollection : IXmlSerializable
    {
        ...
        private XmlSerializableDictionary<string, ScanResult> _lookupTable = new XmlSerializableDictionary<string, ScanResult>();
        ...
        {
            XmlSerializer strings = new XmlSerializer(typeof(string));
            XmlSerializer table = new XmlSerializer(typeof(XmlSerializableDictionary<string, ScanResult>));
            ...
            _lookupTable = (XmlSerializableDictionary<string, ScanResult>)table.Deserialize(reader);
            reader.ReadEndElement();
        ...
        public string GetMD5Hash(string theFile)
        {
            ...
            {
                Console.WriteLine("Failed to hash {0}: {1}", theFile, ex.Message);
            }
        ...
        /// If it exists, the existing value/weight/uniqueness/tag are overwritten
        /// </summary>
        ...
        /// <summary>
        /// If the given name does not exist, it is added
        /// If it exists, the existing value/weight/uniqueness/tag are appended
        /// </summary>
        /// <param name="name"></param>
        /// <param name="value"></param>
        /// <param name="weight"></param>
        /// <param name="uniqueness"></param>
        public void AppendResult(string name, string value, int weight, int uniqueness, object Tag)
        {
            if (ResultExists(name))
            {
                ScanResult result = _lookupTable[name];
                ...
                // no duplicate values
                if (false == result.Value.Contains(value))
                {
                    result.Name = name;
        ...
        public void AppendResult(string name, string value, int weight, int uniqueness)
        {
            AppendResult(name, value, weight, uniqueness, null);
        }
    ...
    public class ScanResultCollectionList : IXmlSerializable
    {
        ...
        {
            ScanResultCollection itemToRemove = null;

            foreach (ScanResultCollection aCollection in Items)
            {
                if (collection.Hash == aCollection.Hash)
                {
                    // duplicate, remove older item
                    itemToRemove = aCollection;
                    break;
                }
            }
