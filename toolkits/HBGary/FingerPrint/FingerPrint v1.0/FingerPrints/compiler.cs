...
namespace FP
{
    class compiler : BaseFingerPrint
    {
        override public bool OnEvaluateString(string theString, ScanResultCollection results)
        {
            ...
            else if (theString.Contains("msvbvm"))
            {
                if (theString.Contains("msvbvm50"))
                {
                    results.AppendResult("Compiler", "Microsoft Visual Basic 5.0", 1, 0);
                }
                else if (theString.Contains("msvbvm60"))
                {
                    results.AppendResult("Compiler", "Microsoft Visual Basic 6.0", 1, 0);
                }
                return true;
            }
            else if (theString.Contains("vb0016.dll"))
            {
                results.AppendResult("Compiler", "Microsoft Visual Basic 4.0 16bit", 1, 0);
                return true;
            }
            else if (theString.Contains("vb0032.dll"))
            {
                results.AppendResult("Compiler", "Microsoft Visual Basic 4.0 32bit", 1, 0);
                return true;
            }
            ...
            Regex r_MSVCRT_dll = new Regex("^MSVBVM(|D).DLL$", RegexOptions.IgnoreCase);
